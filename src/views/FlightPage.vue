            ...
            <v-card v-if="windowWidth > 960" flat class="rounded-lg ">
              <v-row align="center" class="pr-2">
                <v-skeleton-loader class="textLoader d-inline-block mr-2" type="chip" :loading="isLoading">
                  <span class="body-2" style="font-family:Byekan !important">تعداد {{ showTickets.length }} پرواز یافت
                    شد</span>
                </v-skeleton-loader>
                <v-spacer></v-spacer>
                <div>
              ...
              <v-progress-linear class=" mb-4" color="#1abc9c" v-if="loadingTickets" indeterminate
  ...
  methods: {
      ...
      return new Promise(resolve => {
        ...
        if (event.end == 6) {
          if (event.start == 24) {
            ...
            this.showTickets = this.allTickets.filter((x) => {
              return ((x.timeFirst >= 0) && (x.timeFirst <= 6)) || (x.timeFirst >= event.start)
            })
            resolve(this.showTickets)
          }
        } else {
    ...
    async startFilter(event) {
      ...
      this.showTickets = result.filter((x) => {
        return event.airline.includes(x.Airline) && (x.price >= (event.price[0] * 10)) && (x.price <= (event.price[1] * 10))
      });
    },
    getPasanger(event) {
