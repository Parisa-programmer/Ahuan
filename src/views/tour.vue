  ...
  <div class="mt-sm-12 pt-sm-12">
    ...
    <v-row justify="center">
      <div v-if="descriptions.length > 0" class="mt-6 indexDiv">
        <v-skeleton-loader
          class="px-2 px-md-12"
          :loading="loadingMainTour"
          type="chip"
        >
          <h1 v-if="descriptions.length > 0" class="font-small-title-xs">
            {{ descriptions[0].object.title }}
          </h1>
        </v-skeleton-loader>
        <v-skeleton-loader
          class="px-2 px-md-12 mt-4"
          :loading="loadingMainTour"
          type="sentences"
        >
        ...
        <ul
          v-if="descriptions.length > 0 && descriptions[0].type != 'text'"
          class="text-justify answerText mr-6"
        >
          <li v-for="(split, j) in descriptions[0].splitText" :key="j">
            {{ split }}
          </li>
        </ul>
      </div>
      ...
      <div class="indexDiv" :class="loadingMainTour == true ? 'mt-6' : ''">
        <v-skeleton-loader
          ...
          :loading="loadingMainTour"
          type="chip"
          >{{ loadingMainTour == true ? "." : "" }}</v-skeleton-loader
        >
        <v-skeleton-loader
          v-if="descriptions.length > 0"
          class="px-2 px-md-12 mt-8"
          :loading="loadingMainTour"
          type="sentences"
        >
          <div v-for="(item, i) in descriptions" :key="i">
            <h2 v-if="i != 0" class="font-small-title-xs">
              {{ item.object.title }}
            </h2>
            <p
              v-if="i != 0 && descriptions[i].type == 'text'"
              class="text-justify answerText mt-4"
            >
              {{ item.object.description }}
            </p>
            <ul
              v-else-if="i != 0 && descriptions[i].type != 'text'"
              class="text-justify answerText mt-4 mr-6"
            >
...
export default {
  ...
  metaInfo: function () {
    return {
      ...
      meta: [{ name: "description", content: this.descrptionMeta }],
    };
  },
  components: {
    MainTourCard2,
  ...
  methods: {
    getTours(id) {
      let self = this;
      axios
        .get("https://panel.ahuantours.com/api/package/group/notes/" + id)
        .then(function (response) {
          ...
          if (descriptionArray.length > 0) {
            ...
            self.descrptionMeta = descriptionArray[0].object.description.slice(
              0,
              500
            );
          }
    ...
    getCards(id) {
      ...
      let group2 =
        group == "ترکیه"
          ...
          : "";
      axios
        .get("https://panel.ahuantours.com/api/package/group-packages/" + id)
        .then(function (response) {
    ...
    getImage(id) {
      let self = this;
      axios
        .get("https://panel.ahuantours.com/api/package/group/" + id)
        .then(function (response) {
          self.backgroundImage =
            "https://panel.ahuantours.com/uploads/" + response.data.backImage;
